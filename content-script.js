...
// pwhash is a global set by the loader
...
function bind(f) {
    ...
    if ($.inArray(field, fields) != -1 || $(field).hasClass("nopasshash")) {
	return false;
    }
    ...
    function rehash() {
	hash = pwhash.generateHash(input);
    }
...
function onNodeInserted(evt) {
    ...
    $("input[type=password]", evt.target).each(function(index) {
	if (bind(this) && this.id in pwhash.fields) {
...
function update_fields(first) {
    for (var i = 0; i < fields.length; ++i) {
	    fields[i].dispatchEvent(rehashEvt);
    }
    if (first) {
	for (var i = 0; i < fields.length; ++i) {
	    if (fields[i].id in pwhash.fields) {
